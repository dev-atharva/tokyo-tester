...
func (s *Service) processSessionChange(ctx context.Context, change *types.SyncChange, response *types.SyncBatchResponse, userID string) error {
	...
	case "insert", "update":
		...
		sess := &db.Session{
			...
			LeaseExpiresAt:   sessionData.LeaseExpiresAt,
			HeartbeatAt:      sessionData.HeartbeatAt,
			Phase:            sessionData.Phase,
			CheckpointIndex:  sessionData.CheckpointIndex,
			ServiceGraph:     string(sessionData.ServiceGraph),
			TestPlan:         string(sessionData.TestPlan),
			RuntimeSnapshot:  string(sessionData.RuntimeSnapshot),
...
func (s *Service) PullChanges(ctx context.Context, userID string, projectID string) (*types.SyncPullResponse, error) {
	...
	for _, sess := range sessions {
		response.Sessions = append(response.Sessions, types.SessionData{
			...
			LeaseExpiresAt:   sess.LeaseExpiresAt,
			HeartbeatAt:      sess.HeartbeatAt,
			Phase:            sess.Phase,
			CheckpointIndex:  sess.CheckpointIndex,
			ServiceGraph:     json.RawMessage(sess.ServiceGraph),
			TestPlan:         json.RawMessage(sess.TestPlan),
			RuntimeSnapshot:  json.RawMessage(sess.RuntimeSnapshot),
